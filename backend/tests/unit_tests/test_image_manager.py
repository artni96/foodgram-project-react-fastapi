...
async def test_create_random_name(test_base64_fixture):
    ...
    assert isinstance(random_name, str), 'Корректный формат сгенерированного строки - str'
...
async def test_base64_to_file_and_vice_versa(test_base64_fixture):
    ...
    assert os.path.exists(f'src/media/recipes/images/{generated_name}'), 'Не удалось декодировать base64 в файл'

    encoded_image = ImageManager().file_to_base64(generated_name)
    assert encoded_image == test_base64_string, 'Неверная кодировка файла в base64'

    ImageManager().delete_file(generated_name)
    assert not os.path.exists(f'src/media/recipes/images/{generated_name}'), f'Не удалось удалить файл {generated_name}'
