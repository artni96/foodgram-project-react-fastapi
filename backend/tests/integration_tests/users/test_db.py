...
async def test_user_repository(db):
    ...
    assert user_list.result, 'поле result не найдено'
    ...
    assert user_list.count >= len(user_list.result), ('в количество пользователей в result не должно превышать '
                                                      'значение count')
    assert user_list.result[-1].email, 'поле email не найдено'
    assert user_list.result[-1].username, 'поле username не найдено'
    assert user_list.result[-1].id, 'поле id не найдено'
    assert user_list.result[-1].first_name or not user_list.result[-1].first_name, 'поле first_name не найдено'
    assert user_list.result[-1].last_name or not user_list.result[-1].last_name, 'поле last_name не найдено'
    assert user_list.result[-1].is_subscribed or not user_list.result[-1].is_subscribed, 'поле is_subscribed не найдено'
    assert len(user_list.result) <= limit, 'limit при фильтрации не работает'
