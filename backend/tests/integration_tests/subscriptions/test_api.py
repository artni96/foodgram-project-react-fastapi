...
async def test_auth_subsrtibe_flow(auth_ac, ac, db):
    ...
    assert 'recipes_count' in new_sub.json()
    assert isinstance(new_sub.json()['id'], int)
    ...
    assert unfollow.status_code == status.HTTP_401_UNAUTHORIZED

    unfollow = await auth_ac.delete(
        f'/api/users/{users_to_subscribe[-1] + 1}/subscribe'
    )
    assert unfollow.status_code == status.HTTP_404_NOT_FOUND
