...
async def setup_database():
    ...
    ingredients_data = [
        IngredientCreate.model_validate(obj_data) for obj_data in ingredients
    ]

    tags_data = [
        {"name": "Завтрак", "color": "#f5945c", "slug": "breakfast"},
        {"name": "Обед", "color": "#75ba75", "slug": "lunch"},
        {"name": "Ужин", "color": "#be95be", "slug": "dinner"},
    ]
...
async def auth_ac(ac):
    jwt_token = await ac.post(
        ...
        json={"email": "test_user_1@ya.net", "password": "string"},
    )
...
async def another_auth_ac(ac):
    jwt_token = await ac.post(
        "api/auth/token/login",
        json={"email": "test_user_2@ya.net", "password": "string"},
    )
    assert (
        jwt_token.status_code == status.HTTP_201_CREATED
    ), "Статус успешного создания токена должен быть 201"
    assert isinstance(jwt_token.json()["auth_token"], str)
    async with AsyncClient(
        transport=ASGITransport(app=app),
        base_url="http://test",
...
async def recipe_creation_fixture():
    initial_data = {
        ...
        "cooking_time": 10,
        "tags": [2, 3],
        "ingredients": [{"id": 1, "amount": 100}, {"id": 2, "amount": 200}],
...
async def recipe_updating_fixture():
    initial_data = {
        ...
        "ingredients": [{"id": 3, "amount": 300}, {"id": 3, "amount": 400}],
...
async def recipe_bulk_creating_fixture(db, recipe_creation_fixture):
    ...
    for _ in range(len(tags_combinations)):
        ...
        recipe = await db.recipes.create(
            recipe_data=current_recipe, db=db, current_user_id=recipe_author_ids[_]
        )
        if recipe.id % 2 == 1:
            shopping_cart_creation_data = ShoppingCartRecipeCreate(
                recipe_id=recipe.id, user_id=2
            )
            await db.shopping_cart.create(data=shopping_cart_creation_data)
            favorite_recipe_data = FavoriteRecipeCreate(recipe_id=recipe.id, user_id=1)
            await db.favorite_recipes.create(data=favorite_recipe_data)
        else:
            shopping_cart_creation_data = ShoppingCartRecipeCreate(
                recipe_id=recipe.id, user_id=1
            )
            await db.shopping_cart.create(data=shopping_cart_creation_data)
            favorite_recipe_data = FavoriteRecipeCreate(recipe_id=recipe.id, user_id=2)
            await db.favorite_recipes.create(data=favorite_recipe_data)
...
@pytest.fixture()
async def test_recipe(db, recipe_creation_fixture: RecipeCreateRequest):
    recipe = await db.recipes.create(
...
@pytest.fixture()
async def test_recipe_with_all_params(db, test_recipe):
    shopping_cart_creation_data = ShoppingCartRecipeCreate(
        recipe_id=test_recipe.id, user_id=1
    )
    await db.shopping_cart.create(data=shopping_cart_creation_data)
    favorite_recipe_data = FavoriteRecipeCreate(recipe_id=test_recipe.id, user_id=1)
    await db.favorite_recipes.create(data=favorite_recipe_data)
    await db.commit()
    return test_recipe
...
async def test_base64_fixture():
    test_base64_string = (
        "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEBAAABAgMAAABieywaAAAACVBMVEUAAAD///9fX1"
        "/S0ecCAAAACXBIWXMAAA7EAAAOxAGVKw4bAAAACklEQVQImWNoAAAAggCByxOyYQAAAABJRU5ErkJggg=="
    )
