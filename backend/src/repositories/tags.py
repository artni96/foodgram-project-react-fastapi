...
class RecipeTagRepository(BaseRepository):
    ...
    async def create(self, tags_data, db, recipe_id):
        ...
        for tag in tags_data:
            recipe_tags_to_create.append(
                RecipeTagCreate(recipe_id=recipe_id, tag_id=tag)
            )
    ...
    async def update(self, tags_data, db, recipe_id):
        ...
        recipe_tags_to_delete_stmt = delete(RecipeTagModel).filter_by(
            recipe_id=recipe_id
        )
        await self.session.execute(recipe_tags_to_delete_stmt)
        new_tags = await self.create(tags_data=tags_data, db=db, recipe_id=recipe_id)
        return new_tags
