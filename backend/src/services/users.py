...
class UserService(BaseService):
    ...
    async def get_user_by_id(self, id: int, options) -> FollowedUserRead | None:
        user_to_get = await self.db.users.get_one(user_id=id, **options)
        return user_to_get

    async def create_new_user(self, user_data: UserCreateRequest) -> UserCreateResponse:
    ...
    async def change_password(
        self, password_data: UserPasswordUpdate, current_user: UserReadWithRole
    ) -> None:
        ...
        await self.db.users.update(
            id=current_user.id, data=password_updated_data, exclude_unset=True
        )
        await self.db.commit()
